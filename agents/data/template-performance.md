...
Last updated: 2026-07-07 (frs-outreach-writer run).

- Policy mode: **EXPLORATION** (email A/B)
- Scored sends (status=sent AND response_status non-blank): **0** of 528 sent rows / 2082 total log rows
...
Assignment during exploration is deterministic even-split: `idx = (sum of ord(c) for c in prospect_id) % 3`, giving each prospect a reproducible variant so future outcomes stay attributable.
