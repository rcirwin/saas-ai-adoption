...
Last updated: 2026-08-05 (frs-outreach-writer run).

- Total `outreach_log` rows: 2342 (pre-run)
...
| email-short-question | test (brevity) | 116 | 0 | 0 | n/a | n/a |
| email-proof-led | test (credibility-first) | 117 | 0 | 0 | n/a | n/a |
...
**Blocker 2 (found 2026-07-28, still true): the challenger arms have never been sent.** `email-short-question` and `email-proof-led` have now been assigned to 233 drafts between them and not one has ever reached `status = sent`. Every sent email to date used the control. A complete `response_status` backfill would therefore score only `hyper-personalized-email` and still produce no structural A/B read.
...
|---|---|---|
| hyper-personalized-email | 5 | primalogik, qrcodechimp, quickmail, rasayel, recognize-app |
| email-proof-led | 5 | practitest, prodpad, quickschools, receipthero, recruit-crm |
| email-short-question | 2 | repurpose-io, riddle |

The split is deterministic per `prospect_id`, not randomized, so the same prospect always lands on the same variant and outcomes stay attributable across re-drafts. 7 of the 12 emails this run land on a challenger arm, which is the side starved of sends.

## LinkedIn template ranking

Not computable (0 scored sends). Falling back to the default observation-first connect template for all LinkedIn channels regardless of (category, ai_posture). 3 LinkedIn connect drafts this run, all `linkedin-connect-observation-first`, all to truly-never-touched prospects (2 fit-4 plus 1 fit-3) whose `contact_email` is blank.

## Ranking table

Empty. No scored sends to rank. No template bias applied this run.

## Reported inconsistency (re-flag, 9th consecutive run)
