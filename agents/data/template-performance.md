...
- **Mode: EXPLORATION (even split).** Reason: zero scored sends (every `response_status` in `outreach_log` is still blank across 863 log rows pre-run).
...
- This run added 15 `linkedin-connect` drafts (first-touch on the top-15-by-fit never-touched prospects). No `email` drafts this run.
