---
prospect_id: castos
channel: linkedin-connect
template_used: linkedin-connect-default
angle: rogue-startups-peer
date_drafted: 2026-05-14
status: drafted
contact: Craig Hewitt (Founder / CEO) — https://www.linkedin.com/in/craig-hewitt-78386a66
hook_source: Rogue Startups podcast (solo since 2023) + YouTube republishing bet (audio → auto-video) — direct founder-to-founder peer channel, narrow product strategy hook
prior_touch: none — first touch
dossier_decision: SERVICES borderline. Castos has shipped transcription + show notes + social post generation; internal-ops angle (sales, CS, content for a creator-marketing platform) untouched and viable.
hook_distinct_from_v1: n/a — first touch
CONTACT_BLOCKER: LinkedIn URL now in dossier (https://www.linkedin.com/in/craig-hewitt-78386a66) — backfill to prospects.contact_linkedin
---

Hey Craig — caught the Rogue Startups solo arc and the YouTube republishing bet. Curious what audio-to-auto-video adoption is actually doing vs the demo — that's the AI feature with the trickiest pickup curve in creator tools right now. No pitch.
