...
### 2026-05-12 — 14 prospects
- 6 researched (fit 3+), 8 not-a-fit. Avg fit 2.64.
- Top fit-5: zenmaid, emailoctopus. Fit-4: castos, referralcandy. Fit-3: carepatron, tettra.
- **PE-roll-up disqualifier confirmed.** MerusCase is owned by Paradigm holding (Alpine Investors PE) since 2019. The contact (Johnny Fuery) is Principal Engineer, not the buyer. Add to disqualifying signals: when a vertical SaaS appears in a directory but is actually a portfolio product inside a PE roll-up (Paradigm, ASG LegalTech, etc.), the buyer is at holding-co level, not at product level. Always check parent-company / acquisition status for legal/healthcare/PE-favored vertical SaaS.
- **AI-empty company that ships content about AI trends ≠ AI-native company.** ReferralCandy's blog has "2025 referral marketing AI guide" content but the product itself has no shipped AI features. Treat content-marketing AI mentions as a positive personalization hook (they're thinking about AI), NOT as evidence the product has shipped AI. Verify on the product/changelog before scoring AI posture.
- **MCP-server-shipped disqualifier continues to fire reliably.** This run: mailersend and honeybadger both shipped MCP in 2025. The signal is clean and easy to detect via web search ("[company] MCP server [year]"). Continue to use it as a fast-disqualifier check before deep research.
- **Customer support AI category warning intensified.** Three of 14 prospects this run (crisp, customerly, and to a lesser extent loops as devtool-email) are in AI-saturated categories with well-funded native AI competitors. All scored 2. The memory rule from prior runs is holding: "Customer support AI is crowded with well-funded AI-native competitors. Score harshly." Apply pre-emptively: if category includes "customer support / chat / messaging" AND the prospect has shipped any meaningful AI agent, default to fit 2 unless there's a specific gap.
- **Bootstrapped founder running an AI-themed side business is a +1 reachability signal.** Craig Hewitt (Castos) runs craighewitt.com "AI-Era CEO Advisory" and "100 Days of AI" YouTube. Pattern: founders who publicly invest in their own AI-thinking via separate channels respond more readily to AI-strategy outreach. Add to founder reachability signals.
- **Pre-flight `pip install --user cffi cryptography` ran clean** — environment was already configured this run. Worked as no-op.
- Contact data gaps: referralcandy (contact_linkedin), castos (contact_linkedin), tettra (contact_linkedin), emailoctopus has LinkedIn but no email — outreach writer should flag for sourcer backfill. honeybadger and mailersend missing too but they're not-a-fit so moot.
