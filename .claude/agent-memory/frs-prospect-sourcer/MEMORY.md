...
## Run cadence (continued) — 2026-08-04 (all rotation)

- 2026-08-04 (all, rotation + creative fresh-vein sweep): **15 added of 15 requested**. ~50 candidates evaluated, 24 hard DQ, 11 dedup catches. Prospects tab 1571 -> 1586 (1539 -> 1554 unique ids). Source tag `directory`. LINEAR_SKIPPED (no `.mcp.json`, reconfirmed). See `agents/sourcing-runs/2026-08-04-all.md`.
  - Adds: cemsites, purplepass, webcemeteries, crosstrax, qt9, kicksite, bidwrangler, auctionmethod, ecanvasser, jersey-watch, bestnotes, flex-rental-solutions, buildingreports, sbi-software, iris-works.
  - **Fresh veins opened**: (1) **cemetery/deathcare records** — CemSites + webCemeteries, both unfunded founder-led; top of category consolidated (byondpro→OpusXenta, Passare→Funeral Directors Life, PlotBox VC). Still unverified: Chronicle, CFS, FrontRunner, Halcyon. (2) **auction management** — BidWrangler (ex-Amazon engineer, 20 emp, $0 raised) + AuctionMethod (family of auctioneers); Wavebid→Proxibid, Auction Software→Liquidity Services Jan 2025. Still unverified: Maxanet, Bidsquare, AuctionTime. (3) **compliance-inspection SaaS with proprietary datasets** — BuildingReports (13M inspections since 1999, $1.8M/31emp, unfunded) is the archetype; adjacent to the 2026-07-28 backflow/water-compliance vein. Top is VC (Ember Software $10M from 8VC+Menlo Jan 2025) or PE (Brycer/TA, Inspect Point). (4) **behavioral health EHR** — actively PE-rolled (Qualifacts/Ensora/Netsmart), so unfunded survivors like BestNotes (Twin Falls ID, since 2003) stand out. (5) **horticulture/nursery-greenhouse ERP** — SBI Software (100% employee-owned since 2001) joins Picas. (6) **field-canvassing/grassroots advocacy** — Ecanvasser (Cork IE, 14 emp, ~EUR650K raised); CallHub is the over-cap bootstrapped sibling ($7M, Bangalore). (7) **event-production/AV rental ERP** — Flex Rental Solutions (Orem UT, $4M, PPP-only) survives after Current RMS→Klipboard and Rentman's $22M.
  - **Veins checked and CLOSED (add to AVOID)**: marina/boatyard (Molo→Storable, Swell→Oasis Marinas, DockMaster→Valsoft); pet boarding/daycare/pet-sitting (Togetherwork owns Gingr AND PetExec, DaySmart owns Time To Pet, Kennel Connection under 100GROUP); camp management (CampSite→PYXiS, Bunk1+CampMinder→Togetherwork); volunteer management (Better Impact→Brydon Nov 2024, Rosterfy VC, InitLive acquired); process serving (ServeManager→Lawgical→InfoTrack); propane/heating-oil (Blue Cow→PDI); school-bus/student transport (BusHive→Transit Technologies Mar 2025); labor-union mgmt (UnionWare→Valsoft May 2025).
  - **New roll-up parents for the precheck list**: Togetherwork/Aquiline, DaySmart, PYXiS Software Group, Storable, Transit Technologies, The Brydon Group, InfoTrack/Lawgical, Klipboard/Kerridge, PDI Technologies, Proxibid, Liquidity Services, OpusXenta, Arcadea Group (permanent-capital holder — FlightLogger), 100GROUP, Bravo Store Systems (now rolling up pawn: bought all PawnMaster/Data Age assets Jan 2024).
  - **Dev-agency-parent DQ hit again**: GageList was built by Espresso Moon, a Houston custom software dev firm (same shape as LiftKeeper/Advanced Business Technologies last run). Always check "who built it" for tiny bootstrapped tools.
  - **Growjo/ZoomInfo/Kona-Equity revenue estimates are unusable for sub-20-employee companies** — this run saw "Jersey Watch $27M" (9 emp), "Kicksite $25-50M" (40 emp), "GageList $8M" (7 emp), "Iris Works $9.8M" (5 emp). When headcount and revenue estimates are wildly inconsistent, record `arr_estimate=unknown` rather than picking a band. GetLatka remains far more reliable but 403s to WebFetch.
  - Held the line on quality: did NOT pad with Rentopian ($440K, under floor), Coflyt, Aviatize, SpeedyDock, FireLab, Union Impact (all size-unverifiable). Re-check Rentopian + Aviatize next run.

## Environment quirk (added 2026-08-04) — supersedes the 2026-07-28 `pip3 install cffi` note

`scripts/sheet.py` now fails on a fresh session with `ModuleNotFoundError: No module named '_cffi_backend'` **followed by a `pyo3_runtime.PanicException`**. Root cause: default interpreter is python3.11 (`/usr/local/bin/python3`) and the google-auth stack lives in `/usr/local/lib/python3.11/dist-packages`, but the only `cryptography` on the path is the Debian system package in `/usr/lib/python3/dist-packages` built against **cpython-3.12**. A plain `pip install cffi cryptography` reports "already satisfied" and does nothing. Working fix:

```
pip install --force-reinstall --target=/usr/local/lib/python3.11/dist-packages cffi cryptography pycparser
```

`/usr/bin/python3.12` has a working `cryptography` but lacks `google-api-python-client`, so it is not a workaround. Run this FIRST, before any `sheet.py` call, on every new session.

## Run cadence (continued) — 2026-08-11 (all rotation)

- 2026-08-11 (all, rotation + creative fresh-vein sweep): **15 added of 15 default**. ~55 candidates evaluated, ~26 hard DQ, 3 dedup catches (`linentech`, `jasbo`, `rapid-gun-systems` — all looked net-new by name/thesis). Prospects tab 1586 -> 1601 rows (1554 -> 1569 unique ids). Source tag on rows = `directory`. LINEAR_SKIPPED (no Linear MCP tools exposed — reconfirmed; the session MCP preamble still advertises a Linear server with zero tools, do not take it as evidence). The 2026-08-04 `pip install --force-reinstall --target=/usr/local/lib/python3.11/dist-packages cffi cryptography pycparser` fix worked verbatim again — run it first, every session. See `agents/sourcing-runs/2026-08-11-all.md`.
  - Adds: textile-technologies, cyanic-automation, agtrax, forestry-systems, grossman-software, greasebook, avontus, frontrunner-professional, smart145, usked, stone-profit-systems, jetsoft, kudurru-stone, fluency-inc, omti.

### Key learnings 2026-08-11

1. **NEW fresh vein — agribusiness grain accounting / commodity CTRM.** AgTrax (Hutchinson KS, since 1996, *same owner since inception*, ~$5M/30-40emp, 1400+ elevator locations) + Grossman Software Solutions (Champaign IL, since 1976, 11-50emp, AGROSOFT/AGEXCEED). JasBo already in pool. Top of category = ION Group, Bushel, Cultura/Constellation. Grain/feed/agronomy back-office is workflow+data heavy and almost untouched by AI-native entrants.
2. **NEW fresh vein — land-surveying practice management.** Cyanic Automation (Cyanic Job Book, Edmonton, $809.6K/6emp, *unfunded*, Doug Kondor) is the cleanest single fit of the run. Kudurru Stone (Greg Wunz, rodman-turned-developer since 2001, Meridian ID) is the second. SurveyOps (Jeffrey Claxton) shipped its app in late 2025 — sub-floor, re-check in 2027.
3. **NEW fresh vein — aviation MRO for FAA Part 145 repair stations.** Smart 145 (Miami Lakes, founded 2018 by Michael Lopez + Eliurkis Diaz, **23 emp / no funding raised**) is the clean indie. QuantumMX unverified. Ramco/ASA/Quorum are the enterprise tier.
4. **NEW fresh vein — language-access / interpreter-agency operations.** Ūsked (Lyle Vold, DC, no funding) + Fluency Inc (Carmichael CA, since 2005, 50+ agencies). **Boostlingo is the consolidator** — it absorbed Interpreter Intelligence, so precheck any interpreter-tech candidate against it. Still unverified: Anolla, LSP Ware (Atrium), interpreter.io, Caretap.
5. **NEW fresh vein — court-reporting / legal-*support* agency ops.** OMTI Inc (ReporterBase/RB9, founded 1985, hundreds of agencies, Korea dev office) is founder-run and independent. Key insight: **legal-support-services ops software survived the rollups that closed legal *practice* management** (ProfitSolv/AffiniPay/CARET/Paradigm). Still unverified: RepAgencyWorks (Rick Evans), AccuLaw. Acclaim Legal held — PitchBook shows a 2007 acquisition + residential HQ address.
6. **NDT / inspection-data management extends the 08-04 "compliance inspection with proprietary datasets" vein.** JetSoft Ltd (UK, Tom Martin, 4-15emp, ~$5.2M) added under the Cliniko borderline exception. Floodlight Software (fnd. 2019, GRO Incubator, "funding is a challenge") is sub-floor. HUVRdata and DRIVE NDT unverified.
7. **"Operator-turned-vendor" produced 7 of 15 adds** and remains the single highest-precision sourcing-time filter: FrontRunner (Kevin Montroy, 30-yr licensed funeral director + former home owner), Forestry Systems (3-generation sawmill family), Kudurru Stone (rodman), GreaseBook (ex-operator-side), Stone Profit, Textile Technologies (programmer embedded at Star Uniforms), Smart 145 (aviation veterans). Search phrasing that surfaces it reliably: `"<vertical>" software company founder independent bootstrapped <operator-noun> built`.
8. **Founder-succession-to-a-long-time-employee is NOT a DQ.** Textile Technologies: Joe Dulworth retired Jan 2026 after 30+ yrs and sold to Dawn Vaughn, who joined in 2000 in customer service. This reads as "ownership changed 2026" on a naive acquisition precheck. Distinguish employee/family succession from PE/strategic acquisition before DQ-ing.
9. **Sub-floor veins confirmed CLOSED this run (do not re-walk):** commercial beekeeping/apiary (MyApiary 2emp, HiveTracks Reg-CF seed extension, ApiManager/BeeKeepPal micro), driving-school management (DrivingSchool.Software $330K/3emp is the *category leader*), **Wakesys (50 cable parks — now definitively sub-floor, remove from re-check list)**, shooting-range mgmt (ShotPro founders take no salary; Rapid Gun Systems already in pool), scaffolding *inspection* services (not SaaS).
10. **Consolidated veins CLOSED this run:** bowling center mgmt (CDE→QubicaAMF 2016; Steltronic is hardware scoring), axe-throwing/paintball venue (Vantora→Clubspeed 2021; rest is horizontal booking), cattle feedlot/dairy herd (Turnkey = Patterson Companies division, 80emp), fenestration/window-door manufacturing (Cyncly rolled up FeneVision + Window Designer + V6; Windowmaker $21.8M/85emp; Forterro owns EU), 811/utility-locate ticketing (Competers/Utilocate→Urbint; SwiftComply rolling up the rest), repossession/recovery (RDN, Intellaegis $5-25M + institutional round, MBSi), police RMS/municipal court (eFORCE ~$9M/up to 161emp).
11. **New roll-up parents / acquirers for the precheck list: Urbint (damage prevention), Clubspeed (attraction venues), QubicaAMF (bowling), Patterson Companies (animal-health/feedlot), Cyncly (fenestration), Boostlingo (language access), ModivCare (NEMT).** Fullsteam (Maxanet) and Mainsail (Steelhead $84M Dec 2025) both hit again.
12. **WebFetch was unusable for the entire run again** (same as 07-28 and 08-04) — 100% of sourcing and verification ran through WebSearch summaries. GetLatka figures quoted *inside WebSearch snippets* remain the most reliable revenue+headcount signal even though the site 403s on direct fetch (Cyanic $809.6K/6, GreaseBook $2.3M/21, DrivingSchool.Software $330K/3 all came through this path).
13. **AppSumo / Product Hunt rotation is now effectively dead for this ICP.** A 2026 AppSumo deal sweep returned only AI-writing/SEO micro-tools (Writingmate, Writecream, ClickRank, TeamPal) plus AppSumo Originals — zero ICP fits. Spend the rotation budget on vertical fresh-vein sweeps instead and note the AppSumo pass in the run file.

### Next under-sampled to mine (updated 2026-08-11)
- Still-fresh / partially-mined: language-access remainder (Anolla, LSP Ware/Atrium, interpreter.io, Caretap — precheck vs Boostlingo), court-reporting remainder (RepAgencyWorks, AccuLaw), NDT remainder (HUVRdata, DRIVE NDT), grain/agri remainder (Vortract, AgVantage, Solentra — verify scale), stone/countertop remainder (Stonify, Stone Project), aviation MRO remainder (QuantumMX), moving & storage ops (Movegistics/Netensity, MovingPro.io, MoveNinja — unverified, MoveHQ is the consolidator), water-well drilling (DrillerDB, MyWaterWell — likely near-floor), seafood/produce traceability (BlueTrace Maine, inecta — unverified), plus prior open: bakery ERP (Streamline, Bakery Cloud), meat processing (Forkto UK, Custom Meat Solutions), auto salvage-yard (AutoRecycler.io/Ario), portable-restroom (PumperPRO, Septic BizMan), firearm/FFL (FFLAssist), cemetery/deathcare (Chronicle, CFS, Halcyon), auction (Bidsquare), leisure operator-turned-vendor (mini-golf, karting — but Clubspeed is now a confirmed acquirer).
- AVOID (added this run): commercial beekeeping/apiary, bowling center mgmt, axe-throwing/paintball venue, cattle feedlot & dairy herd, fenestration/window-door manufacturing, driving-school mgmt, 811/utility-locate ticketing, repossession/recovery, police RMS & municipal court, uniform-rental *enterprise* tier — plus the full prior AVOID list.
